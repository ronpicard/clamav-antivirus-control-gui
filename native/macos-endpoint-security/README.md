...
- **Endpoint Security clients** run in a **System Extension** (or equivalent privileged target), not inside the Tauri shell or the Node helper.
...
5. **UI integration** (optional): the Tauri app could later **discover** or **configure** the helper (paths, enable/disable) via a local socket or `launchd` — the extension itself cannot be "just npm installed."
